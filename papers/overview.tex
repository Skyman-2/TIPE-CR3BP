...
\begin{document}
	...
	\begin{equation}
		...
		\end{bmatrix}
	\end{equation}
	et on peut maintenant chercher les valeurs propres de cette matrice, de manière à obtenir une solution de l'équation différentielle $\dot{X} = AX$ que l'on peut visualiser clairement dans un espace de phase à l'aide de sa version diagonalisée. Il sera important de noter que les axes de l'espace de phase correspondront à la base des vecteurs propres.
	...
	\end{cases}
	\end{equation}
	ce qui, en notant $Z(P)$ les racines de $P \in \mathbb{R}[X]$, s'écrit simplement $Z(\chi_A) = \{\lambda, -\lambda, i\nu, -i\nu\}$. Et comme on est en dimension finie (à savoir 4 ici), les racines de $\chi_A$ sont exactement les valeurs propres de $A$ : $Sp(A) = Z(\chi_A)$. Il existe alors une base de vecteurs propres que l'on note $B_{p} := (\xi, \eta, \zeta_1, \zeta_2)$ que l'on utilisera aussi comme système de coordonnées pour représenter le mouvement autour de L$_i$. On peut alors écrire
	\begin{equation}
		A
		=
		P
		...
		P^{-1}
	\end{equation}
	avec $P$ une matrice de passage à une base de vecteurs propres.
