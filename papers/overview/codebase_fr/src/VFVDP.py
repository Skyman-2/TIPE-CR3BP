# Velocity-First Velocity Dependant Position Integration method (cf Braintruffle latest video)
import numpy as np
import time
...
def simuler_trajectoire(conditions_initiales,temps_simulation,pas,systeme):
    ...
    while t < t_max:
        ...
        V_new = np.array([V[i][0] + equation_x(P[i][0],P[i][1],V[i][1]) * dt, V[i][1] + equation_y(P[i][0],P[i][1],V[i][0]) * dt])
        P_new = np.array([P[i][0] + V_new[0] * dt, P[i][1] + V_new[1] * dt])
