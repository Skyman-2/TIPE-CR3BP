...
\begin{document}
	...
	\begin{equation}
		...
		\end{cases}
	\end{equation}
	et on résout le système de Cramer $2\times 2$ défini par les lignes 3 et 4, or si le déterminant de ce système est non-nul, alors $x_1 = x_2 = 0$ et on veut des solutions non-nulles, donc les lignes du système sont liées, et avec la ligne 3 par exemple, on a
	\begin{equation*}
	...
	\section{Dégénérescence des orbites autour de L1 et L2}
	Il a été établi dans la partie précédente que les points L1 et L2 ne sont pas des points d'équilibre stable, mais des points d'équilibre de type "col", ou encore "selle". Un théorème de Lyapunov, souvent connu sous le nom de \textit{Lyapunov Center Theorem}, démontre l'existence d'une famille d'orbites dite de Lyapunov dans le plan sous réserve de vérification des hypothèses. Ces orbites sont instables. On mènera alors une étude dans le cadre de la théorie de Floquet qui visera à caractériser la dégénérescence de ces orbites.
	
	\subsection{Existence d'orbites périodiques}
	\begin{theorem}{du centre de Lyapunov}
		Soit $\dot{x} = H(x)$ un système Hamiltonien de $\mathbb{R}^{2n}$, $x_0$ une position d'équilibre identifiée à $0$, $A = \frac{\partial H}{\partial x}(0)$ la matrice du système linéarisé. On suppose que le spectre de A est de la forme $Sp(A) = \{ \pm i\omega, \lambda_3, \dots, \lambda_{2n} \}$, où $\omega > 0$. Si $\forall j \in [|3,2!n|], \frac{\lambda_j}{iw}\notin\mathbb{Z}$, alors il existe une famille à un paramètre d'orbites périodiques issues de 0.
	\end{theorem}
