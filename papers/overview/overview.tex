...
\begin{document}
	...
	\subsection{Introduction des variétés}
	\textbf{Définition :} Soit un système différentiel $\dot{X} = F(X)$ où $F \in \mathcal{C}^1(\mathbb{R}^n, \mathbb{R}^n)$. Pour une condition initiale $x_0$, on pose $\varphi(t,x_0)$ l'état du système à un instant $t$ tel que $\varphi(t_0,x_0)=x_0$. Soit $\gamma \in \mathcal{F}(\mathbb{R}, \mathbb{R}^{2n})$ une orbite de référence de période $T$. Alors on définit la variété \textit{stable} (resp. \textit{instable}) associée à $\gamma$ notée $W^s(\Gamma)$ (resp. $W^u(\Gamma)$) comme
	\begin{equation*}
