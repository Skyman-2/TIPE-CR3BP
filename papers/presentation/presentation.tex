...
\begin{document}
	...
	\subsection{Système, potentiel apparent}
	
	\begin{frame}
		\begin{figure}
				\includegraphics[width=0.75\linewidth]{RNG_diag.pdf}
				\caption{Schéma du système dans le référentiel $R_{NG}$.}
				\label{diagram rng}
		\end{figure}
	\end{frame}
	...
	\begin{frame}
		\begin{figure}
			\includegraphics[width=0.8\linewidth]{potentiel_apparent_3D.png}
	...
	\begin{frame}
		...
		\begin{figure}
			...
			\caption{Tracé du potentiel apparent $x \mapsto \bar{U}(x,0)$ avec Desmos.}
			\label{tracé du potentiel 3D python}
		\end{figure}
	\end{frame}
	...
	\begin{frame}
		...
		avec $\lambda = \omega\sqrt{1+2\sqrt{7}} \text{, }\nu = \omega\sqrt{2\sqrt{7}-1}$.
		
				
		Exemples de temps de dégénerescence caractéristique :
		\begin{equation*}
			\begin{cases}
				\frac{1}{\lambda_{Terre/Lune}} = 1.8 \text{ jours}
				\\
				\frac{1}{\lambda_{Soleil/Terre}} = 23 \text{ jours}
			\end{cases}
		\end{equation*}
	\end{frame}
	...
	\section{Orbites autour de L1 et L2}
	
	\subsection{Existence d'orbites planaires}
	\begin{frame}
		\begin{block}{Définition : Système Hamiltonien}
			
		\end{block}
		\begin{block}{Théorème de Lyapunov-Poincarré}
			\textcircledd{H} Soit $x_0$ une position d'équilibre dans un système Hamiltonien
		\end{block}
	\end{frame}
