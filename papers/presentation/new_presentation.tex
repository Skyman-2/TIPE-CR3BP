...
\begin{document}	
	...
	\begin{frame}
		...
		Comprendre comment la structure hyperbolique des orbites périodiques autour de L1 et L2 engendre des variétés invariantes utilisables pour concevoir des trajectoires spatiales peu coûteuses en carburant.
	\end{frame}
	...
	\begin{frame}
		\frametitle{Simulation de la divergence orbitale autour de L1}
		\begin{figure}
			\centering
			\includegraphics[width=0.75\linewidth]{degenerescence_L1_plots_RK4.png}
			\label{fig:instability simulation}
			\caption[width=0.75\linewidth]{Simulation numérique par schéma de Runge-Kutta 4 de la divergence de l'orbite autour de L1.}
		\end{figure}
	...
	\begin{frame}
		...
		\begin{mydefinition}{Matrice fondamentale}
			Soit $(y_1,\dots,y_n)$ un système fondamental de solutions d'une équation différentielle linéaire. Une matrice fondamentale de cette équation différentielle est l'application $t \mapsto \Phi(t)$ où $\Phi(t)$ est la matrice dont les colonnes sont les vecteurs $(y_1(t),\dots,y_n(t))$.
	...
	\begin{frame}
		...
		\begin{mydefinition}{Le flot}
			Soit un système non-linéaire $X' = f(X)$ où $f \in \mathcal{C}^1(\mathbb{R},\mathbb{R}^4)$. On définit le flot comme l'application $ \eta :
			\begin{array}{rcl}
	...
	\begin{frame}
		...
		{\small Soit $\gamma$ une solution $T$-périodique du système non-linéaire. Soit $\delta x_0 \in \mathbb{R}^4$ une petite perturbation initiale. On linéarise le flot le long de $\gamma$ :
		\begin{equation*}
			\eta(t,x_0+\delta x_0) = \eta(t,x_0) + D_{x_0}\eta(t,x_0)\delta x_0 + \underset{\delta x_0 \to 0}{o}(\delta x_0)
		\end{equation*}
		\pause		
		et on remarque que $D_{x_0}\eta(t,x_0)$ vérifie 
		\begin{equation*}
			\Phi ' = Df(\gamma(t))\Phi, \text{ et } D_{x_0}\eta(0,x_0) = I_4
		\end{equation*}
		\pause
	...
	\begin{frame}
		...
		\begin{importantresult}{}
			Si $\Phi$ et $\Psi$ sont deux matrices fondamentales d'une même équation différentielle. Alors il existe $M \in GL_n(\mathbb{C})$ tel que $\Phi = \Psi M$ et que $M$ soit indépendante de $t$.
		\end{importantresult}
		\textit{Démonstration :}
	...
	\begin{frame}
		...
		\begin{importantresult}{Lien matrice de Monodromie/sensibilité aux C.I.}
			...
			\end{equation*}
		\end{importantresult}
		Pour $x_0 \in \mathbb{R}^n$ une condition initiale, $t \in \mathbb{R}$ un instant, et $\delta x_0 \in \mathbb{R}^n$ une petite perturbation, on pose
		\begin{equation*}
			\delta x(t) := \eta(t,x_0 + \delta x_0) - \eta(t,x_0)
		\end{equation*}
		qui représente la variation de la trajectoire en $t$ pour une perturbation $\delta x_0$.
		...
		\begin{columns}
			...
			On a que $D_{x_0}\eta(t,x_0) = \Phi(t)$ comme démontré dans la présentation. Et en $t = T$, on obtient bien
			\begin{equation*}
				\delta x(T) = \Phi(T) \delta x_0 = M \delta x_0
			\end{equation*}
	...
	\begin{frame}
		...
		\begin{equation*}
			M^k = (e^{TB})^k = e^{kTB} = P(kT)e^{kTB} = \Phi(kT)
		\end{equation*}
		}
	\end{frame}
